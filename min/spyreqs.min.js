(function(window){"use strict";var appUrl,hostUrl,queryParams,executor,baseUrl,targetStr,notAnApp_FlagSum=0,webPropertiesStorage={"appWebProperties":{isUnloaded:true},"hostWebProperties":{isUnloaded:true},},say,rest,jsom,inAppMode=true,spyreqs,spyreqs_version="0.0.28";if(!(window.performance)){window.performance={};}
...
function prepRest(){targetStr="&@target='"+hostUrl+"'";baseUrl=appUrl+"/_api/SP.AppContextSite(@target)/";executor=new SP.RequestExecutor(appUrl);}
...
function tryBuildAppUrl(){if(!window.location.origin){window.location.origin=window.location.protocol+"//"+window.location.hostname+(window.location.port?':'+window.location.port:'');}
...
function iAmNotInApp(){initModeMsg="no app";inAppMode=false;var url=window.location.href;appUrl=hostUrl=url.substring(0,url.indexOf('/Pages'));if(appUrl.length<1){appUrl=hostUrl=url.substring(0,url.indexOf("?"));}
if(appUrl.length>1){$.getScript(hostUrl+"/_layouts/15/SP.RequestExecutor.js").done(function(script,textStatus){say('loaded: RequestExecutor.js');executor=new SP.RequestExecutor(hostUrl);targetStr="&@target='"+hostUrl+"'";baseUrl=appUrl+"/_api/SP.AppContextSite(@target)/";}).fail(function(script,textStatus){say('could not load: RequestExecutor.js');});if(!SP.ClientContext){say("spyreqs is waiting for sp.js");initTimer=setInterval(testReady,500);}else{say('sp.js is already loaded')
if(typeof window.onSpyreqsReady=='function')window.onSpyreqsReady();}}else{say("spyreqs not in SharePoint");}}
...
function testReady(){if(!isReady){SP.SOD.executeFunc('sp.js','SP.ClientContext.get_current',function(){say('loaded: sp.js');if(!isReady){if(typeof window.onSpyreqsReady=='function')window.onSpyreqsReady();isReady=true;}
...
if(!inAppMode){context=new SP.ClientContext.get_current();}else{context=new SP.ClientContext(appUrl);}
...
params[param]=val;var attached="?";for(var key in params){attached+=key+"="+params[key]+"&";}attached=attached.substr(0,attached.length-1);return String(str.substr(0,ind)+attached);}return String(str+"?"+param+"="+val);}
...
function fail(sender,args){var error={sender:sender,args:args};defer.reject(error);}
return defer.promise();},removeRecentElemByTitle:function(c,elemTitle,literalsArray){var defer=new $.Deferred();literalsArray=literalsArray||["Recent"];if(!(literalsArray instanceof Array))literalsArray=[literalsArray];function success(){var msg='element removed from Recent: '+elemTitle;defer.resolve(msg);}
function fail(sender,args){var error={sender:sender,args:args};defer.reject(error);}
...
if(answerBool==-1){var args={get_message:function(){return"List was not found: "+elemTitle;},get_stackTrace:function(){return null;}};fail(null,args);}
else{defer.resolve(answerBool);}}
function fail(sender,args){var error={sender:sender,args:args};defer.reject(error);}
