(function (window) {
	...
	function newRemoteContextInstance() {
		...
		appContextSite;
		if (!SP.ClientContext) {
			say("SP.ClientContext not loaded");
			return null;
		}
	...
	function newLocalContextInstance() {
		...
		if (!SP.ClientContext) {
			...
			SP.SOD.executeFunc('sp.js', 'SP.ClientContext.get_current',
				function () {
				say('loaded: sp.js, spyreqs ready');
			});
			return null;
		}
		if (!inAppMode) {
	...
	jsom = {
			...
			if (typeof listObj.title === 'undefined') {
				...
				var args = {
					get_message : function () {
						return "createList cannot create without .title";
					},
		...
		removeRecentElemByTitle : function (c, elemTitle, literalsArray) {
			...
			if (!(literalsArray instanceof Array))
				literalsArray = [literalsArray];
			...
			c.context.executeQueryAsync(
				function () {
				var objEnumerator = ql.getEnumerator(),
				navItem,
				recentsFoundBool,
				navItemTitle;
				while (objEnumerator.moveNext()) {
					...
					if ($.inArray(navItemTitle, literalsArray) !== -1) {
						...
						c.context.executeQueryAsync(
							function () {
							var childsEnum = ch.getEnumerator(),
							childItem,
							foundBool = false;
							...
							if (!foundBool) {
								var args = {
									get_message : function () {
										return "Recent Element was not found: " + elemTitle;
									},
									get_stackTrace : function () {
										return null;
									}
				...
				if (!recentsFoundBool) {
					var args = {
						get_message : function () {
							return "Recent Node was not found: " + elemTitle;
						},
						get_stackTrace : function () {
							return null;
						}
		...
		getListItemHasUniquePerms : function (c, listTitle, itemIdOrFilename) {
			...
			function success() {
				var answerBool,
				itemTitle,
				listEnumerator = resultCollection.getEnumerator();
				while (listEnumerator.moveNext()) {
		...
		getListHasUniquePerms : function (c, listTitle) {
			...
			function success() {
				...
				if (answerBool == -1) {
					var args = {
						get_message : function () {
							return "List was not found: " + elemTitle;
						},
						get_stackTrace : function () {
							return null;
						}
		...
		createFile : function (c, doclib, fname, body, overwriteBool) {
			...
			function successHandler() {
				defer.resolve(newFile.get_serverRelativeUrl());
			}
	...
	spyreqs = {
		...
		rest : {
			...
			breakRoleInheritanceOfHostList : function (listTitle, copyRolesStr, clearSubScopeStr) {
				var defer = new $.Deferred(),
				copyRolesStr = copyRolesStr || "true",
				clearSubScopeStr = clearSubScopeStr || "false",
		...
		jsom : {
			...
			createHostList : function (listObj) {
				...
				fields : [{"Name":"userId", "Type":"Text", "Required":"true"},{"Name":"scoreFinal", "Type":"Number", "hidden":"true"},{"Name":"assginedTo", "Type":"User"},{"Name":"state", "Type":"Choice", "choices" : ["rejected", "approved", "passed", "proggress"]},{"Name":"comments", "Type":"Note"}
				]
				})
				.then( ...... )
			...
			createHostSite : function (title, template, url, inheritPermsBool, language, description) {
				/* example  templateGuid = "{0D760005-1A24-439F-AC8C-B123C44E46E1}";
				language = 1033; // or could be null				*/
				var defer = $.Deferred(),
				c = spyreqs.jsom.getTestHostContext(),
				websColl,
				webCreationInfo,
				newWeb;
				websColl = c.appContextSite.get_web().get_webs();
				webCreationInfo = new SP.WebCreationInformation();
				webCreationInfo.set_title(title);
				webCreationInfo.set_webTemplate(templateGuid);
				webCreationInfo.set_url(url);
				webCreationInfo.set_description(description);
				webCreationInfo.set_language(language);
				webCreationInfo.set_useSamePermissionsAsParentSite(inheritPermsBool);
				newWeb = websColl.add(webCreationInfo);
				c.context.load(newWeb);
				c.context.executeQueryAsync(
					function () {
					defer.resolve(true);
				}, function (sender, args) {
